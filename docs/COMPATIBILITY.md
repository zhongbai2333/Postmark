...
活动约定 `stamp_id=expert` 为大师章，`visitor` 为普通章；只改变工具样式，原图案不变。0.1.3 起，收藏身份使用展区 UUID、章 ID 与 item，跨展区相同图片不会合并；同一章 ID 的不同图案分别保存，重复盖章台使用相同 item 时去重。
...
异步章面任务按图案使用独立票据，同槽换图不覆盖其他图案的任务；清空后旧任务不得恢复收藏，再次领取相同章面会启动新任务。已拿取但失去收藏资格的工具和未提交印迹会取消，已经落纸的印迹保留。
