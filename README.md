...
2. 从 [Releases](https://github.com/zhongbai2333/Postmark/releases) 下载已发布的 Postmark JAR，放入客户端实例的 `mods` 文件夹。不要安装 `-sources.jar`。当前正式版本为 `0.1.3`，构建产物见 `build/libs/`。
...
TeaCon 2026 玩家盖章表已内置为预设：74 个展馆、112 枚章按玩家确认的导出记录匹配，单章馆一个剪影、双章馆两个；实际发现后显示淡色章面，领取后点亮并快速判断集齐。清单不授章、不写入脚印；来源、未匹配记录与完整性边界见 [预设说明](docs/STAMP_CATALOG.md)。
