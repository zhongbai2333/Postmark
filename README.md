...
- 大幅拼贴《漫游志》将全部 SMU 展馆放在同一张可拖动、缩放的纸面；悬停纸片平滑放大，木柄普通章和金柄大师章分列两角，附近检索后留下脚印。
...
2. 从 [Releases](https://github.com/zhongbai2333/Postmark/releases) 下载已发布的 Postmark JAR，放入客户端实例的 `mods` 文件夹。不要安装 `-sources.jar`。当前源码版本为 `0.1.0-alpha.18`，本地构建产物见 `build/libs/`。
...
Windows 使用 `./gradlew.bat build`，需要 JDK 25。当前 **46 项单元测试**，另有真实 Minecraft 工作台和 SMU 集成服务器测试。命令、夹具及验证边界见 [CONTRIBUTING.md](CONTRIBUTING.md) 和 [验证记录](docs/VALIDATION.md)。
